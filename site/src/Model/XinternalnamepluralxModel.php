...
/**
 * Methods supporting a list of Custom records.
 *
 * @since  1.0.0
 */
class XinternalnamepluralxModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		if (!Factory::getApplication()->getIdentity()->authorise('core.edit', 'com_custom'))
		{
		...
		else
		{
			...
			if (!empty($search))
			{
				...
				else
				{
					...
					$query->where('( a.xtextfieldx LIKE ' . $search . '  OR  a.xtextareax LIKE ' . $search . ' )');
				}
			}
	...
	public function getItems()
	{
		...
		foreach ($items as $item)
		{
				...
				$options      = explode(',',$item->xcheckboxesx);
				$options_text = array();
				...
				$item->xcheckboxesx = !empty($options_text) ? implode(',', $options_text) : $item->xcheckboxesx;

			// Get the title of every option selected
			$options = array();
			if (!empty($item->xlistx)) {
				$options = explode(',', $item->xlistx);
			}
			$options_text = array();
			...
			$item->xlistx = !empty($options_text) ? implode(',', $options_text) : $item->xlistx;

				if (!empty($item->xgrouplistx))
					{
						$item->xgrouplistx = Text::_('COM_CUSTOM_XINTERNALNAMEPLURALX_XGROUPLISTX_OPTION_' . preg_replace('/[^A-Za-z0-9\_-]/', '',strtoupper(str_replace(' ', '_',$item->xgrouplistx))));
					}

			if (isset($item->xsqlqueryx))
			{
				$values    = explode(',', $item->xsqlqueryx);
				$textValue = array();
				...
				$item->xsqlqueryx = !empty($textValue) ? implode(', ', $textValue) : $item->xsqlqueryx;
			}
		}
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(Text::_("COM_CUSTOM_SEARCH_FILTER_DATE_FORMAT"), "warning");
